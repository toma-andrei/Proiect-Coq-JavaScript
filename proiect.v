...
| eqtoEval : forall a1 a2 i1 i2 state comp,
  ...
  a2 =[ state ]=> i2 ->
  comp = Nat.eqb i1 i2 ->
  (beqto a1 a2) =[ state ]=> if (comp) then 1 else 0
...
Example evalProgram : exists state, program ={ stateFirst }=> state /\ state a = 4.
Proof.
  eexists.
  split.
  unfold program.
